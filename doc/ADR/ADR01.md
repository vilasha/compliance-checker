...
3. **LLM Strategy**  
   - Base model: **`llama3.2:8b`** (lightweight, multilingual).  
...
#### 3. **RAG Orchestration (LangChain4j)**
   ...
   - **LLM Call**: Send to Ollama’s fine‑tuned `llama3.2:8b` model.
...
#### 12. **Local Development Environment**
   - Docker Compose with:
     ...
     - `ollama/ollama` (pre‑pull `llama3.2:8b`, `bge-m3`, and fine‑tuned model)
...
| LLM                 | llama3.2:8b fine‑tuned on rcds/swiss_rulings, served by Ollama           |
